...
# Indexes for the start and end of the unsubscribe portion
unsubscribe_start = 7
unsubscribe_stop = 13
...
tests = {
  ...
  "c1_unsubscribe_partial": "not_executed",
  "c1_stop_2": "not_executed",
  "c1_start_2": "not_executed",
  "c1_check_subscriptions": "not_executed",
...
def start_and_check_client(server, id, test_id="", restart=False, test=True):
  """Starts a TCP client and checks that it starts."""
  if test:
    fail_test("c" + id + ("_restart" if restart else "_start") + (f"_{test_id}" if len(test_id)>0 else ""))
  ...
  if success and test:
    pass_test("c" + id + ("_restart" if restart else "_start") + (f"_{test_id}" if len(test_id)>0 else ""))
...
def run_test_c1_start(server, test_id):
  """Tests that a subscriber C1 starts correctly."""
  return start_and_check_client(server, "1", test_id)
...
def run_test_c1_unsubscribe_partial(c1, topics):
    fail_test("c1_unsubscribe_partial")
    print("Unsubscribing C1 from some topics")

    failed = False
    for topic in topics[unsubscribe_start:unsubscribe_stop]:
        c1.send_input("unsubscribe " + topic.name)
        outc1 = c1.get_output_timeout(1)
        if not outc1.startswith("Unsubscribed from topic"):
            print("Error: C1 not unsubscribed from a topic")
            failed = True
            break
    ...
    # Check that C1 receives all the messages correctly
    success = True
    for topic in topics[:unsubscribe_start]:
        success =  check_subscriber_output(c1, "1", topic.print()) and success

    for topic in topics[unsubscribe_stop:]:
        success =  check_subscriber_output(c1, "1", topic.print()) and success

    if success:
        pass_test("c1_unsubscribe_partial")

def run_test_c1_check_subscriptions(c1,topics):
    fail_test("c1_check_subscriptions")
    print("Checking if the subscriptions are still correct when reconnecting")
    ...
    if success:
        pass_test("c1_check_subscriptions")
...
def h2_test():
  ...
  c1, success = run_test_c1_start(server,"1")
  ...
  if success:    
    ...
    success = run_test_c1_stop(server, c1, "1")
    ...
    if success:
      ...
      if success:
        ...
        if success:
          ...
          # Unsubscribe from some of the topics
          run_test_c1_unsubscribe_partial(c1, topics)
                    
          # Stop C1 and check if it exists correctly
          run_test_c1_stop(server,c1,"2")
          
          # Start C1 and check that it starts properly
          c1, success = run_test_c1_start(server,"2")
          
          # Check if the subscriptions remained correct when moving the client
          run_test_c1_check_subscriptions(c1, topics)
